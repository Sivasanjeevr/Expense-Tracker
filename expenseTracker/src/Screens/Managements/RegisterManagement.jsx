...
const RegisterManagement = ({navigation}) => {
  ...
  const {registerManagement} = useContext(AuthContext)
  ...
  handleSubmit = async()=>{
    if(password!=confirmPassword){
      ...
      const result = await registerManagement(name,mobileno,password,confirmPassword,email)
      if (result === "success") {
  ...
  return (
    <View style={{flex:1,justifyContent:'center',alignItems:'center',rowGap:30}}>
      ...
      <Pressable onPress={()=>{navigation.navigate('login-management')}}>
